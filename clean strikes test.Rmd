...
d <- data %>% filter(year(DATE) %in% c(2000:2005,2010:2018)) %>% dplyr::select(-c(DAYOFWEEK, SEASON, YEAR)) #%>% na.omit() 
d <- d %>% 
...
### --6-- Risk = percentile of average of strike chance across all months of the year
###       Label attached to all days in a week based on how risky the specific week was historically
...
### --7-- Risk = percentile of average of strike chance across all months of the year
###       Label assigned to all days based on where they rank in that lookup
...
x <- d %>% 
  ...
  separate(RISK, into=c("L","H"), sep=",", remove=FALSE) %>% 
  mutate(L=as.numeric(L), H=as.numeric(H)) %>%
  arrange(L) %>%
  distinct(L, .keep_all = TRUE)

# x is now a lookup table
findInterval(0.64, x$L)

# go through each data point and assign it based on the lookup
d$RISK <- findInterval(d$RATIO, x$L)
# FIX - BIG KLUDGE
d$RISK <- ifelse(d$RISK == 0,0,d$RISK-1)
xQuantDayMonthHist <- d
...
xgTrain <- dplyr::select(dfTrain, -c(DATE,STRIKECOUNT,STRIKE, RISK, YEAR, RATIO)) %>% 
  mutate(FOG = as.numeric(FOG),RAIN_DRIZZLE = as.numeric(RAIN_DRIZZLE),
         SNOW_ICE = as.numeric(SNOW_ICE), HAIL = as.numeric(HAIL), THUNDER=as.numeric(THUNDER))

xgTest <- dplyr::select(dfTest, -c(DATE,STRIKECOUNT, STRIKE, RISK, YEAR, RATIO)) %>% 
  mutate(FOG = as.numeric(FOG),RAIN_DRIZZLE = as.numeric(RAIN_DRIZZLE),
         SNOW_ICE = as.numeric(SNOW_ICE), HAIL = as.numeric(HAIL), THUNDER=as.numeric(THUNDER))
...
xgTest$ACTUAL = dfTest$RISK
xgTest$PREDICTED = xgbMulti$RISK
...
# Model seasonality
data <- readRDS("~/WTFProject/Birdstrikes_Models/KDEN_FINAL_NEW.RDS")
d <- data %>% filter(year(DATE) %in% 2000:2014) %>% dplyr::select(-c(DAYOFWEEK, SEASON, YEAR)) #%>% na.omit() 
d <- d %>% 
  mutate(WEEK=week(DATE),WEEKDAY= wday(DATE), DAY = yday(DATE), YEAR=year(DATE), MONTH = as.numeric(MONTH)) %>%
  mutate(RATIO=STRIKECOUNT/FLIGHTCOUNT*10000)
...
x <- d %>% 
  mutate(RATIO=STRIKECOUNT/FLIGHTCOUNT*10000) %>%
  group_by(MONTH) %>%
  summarise(RATIOM = mean(RATIO)) #%>%  mutate(RISK = .bincode(RATIO, breaks = quantile(RATIO, probs = seq(0,1,1/3)), include.lowest=TRUE)-1)


xM <- left_join(d,x, by=c("MONTH"="MONTH"))  

ggplot(xM[xM$YEAR==2010,],aes(DATE,RATIO)) + geom_line() + geom_point(aes(DATE,RATIOM), colour="blue")

xM$DELTA <- abs(xM$RATIOM-xM$RATIO)
xM$RISK = .bincode(xM$DELTA,breaks = quantile(xM$DELTA, probs = seq(0,1,1/3)), include.lowest=TRUE)-1

dd <- as.data.frame(xM)
trainyn <- createDataPartition(xM$RISK, p=0.75, list=FALSE)
dfTrain <- dd[trainyn,]
dfTest <- dd[-trainyn,]
rm(trainyn)

plot(xM$DELTA[xM$YEAR==2014])
hist(xM$DELTA)

xgTrain <- dplyr::select(dfTrain, -c(DATE,STRIKECOUNT,STRIKE, YEAR, RISK, RATIO, RATIOM, DELTA)) %>% 
  mutate(FOG = as.numeric(FOG),RAIN_DRIZZLE = as.numeric(RAIN_DRIZZLE),
         SNOW_ICE = as.numeric(SNOW_ICE), HAIL = as.numeric(HAIL), THUNDER=as.numeric(THUNDER))
xgTest <- dplyr::select(dfTest, -c(DATE,STRIKECOUNT,STRIKE, YEAR, RISK, RATIO, RATIOM, DELTA)) %>% 
  mutate(FOG = as.numeric(FOG),RAIN_DRIZZLE = as.numeric(RAIN_DRIZZLE),
         SNOW_ICE = as.numeric(SNOW_ICE), HAIL = as.numeric(HAIL), THUNDER=as.numeric(THUNDER))
