...
model.data.day <-
  model.data.day %>% filter(model.data.day$YEAR %in% 2014:2018) ## CE CHANGED
...
fit2 <- randomForest::randomForest(
  as.factor(STRIKE) ~ WSPEED + WANGLE + BIRDCOUNT + FLIGHTCOUNT + MONTH + TEMP + COVERH + CLOUDH + VIS + SLP + PRECIP + SEASON + DAY,
  data = model.data.day,
  mtry = 2,
  importance = TRUE,
  proximity = TRUE,
...
fit3 <- randomForest::randomForest(
  as.factor(STRIKE) ~ WSPEED + WANGLE + BIRDCOUNT  + MONTH + TEMP + COVERH + CLOUDH + VIS + SLP + PRECIP + DAY,
  data = model.data.day,
  mtry = 2,
  importance = TRUE,
  proximity = TRUE,
...
### FIT 2 seems to have only 3.7% classification error for FN incl. COVER? NOTE FLIGHTCOUNT IS WRONG
print(fit3)

varImp(fit)
randomForest::varImpPlot(fit2, type = 2)
...
y <- "STRIKE"
x <- setdiff(names(model.data.day), c("STRIKE", "STRIKECOUNT", "YEAR", "DAY", "SEASON"))
...
model.cla <- c(setdiff(names(model.data.day), c("STRIKECOUNT")))
...
model.reg <- c(setdiff(names(model.data.day), c("STRIKE")))
