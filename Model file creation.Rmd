...
m <- left_join(empty, df, by=c("AIRPORT ID" = "AIRPORT ID", "DATE"="DATE","RTIME"="RTIME")) %>%
  dplyr::select(`AIRPORT ID`, `DATE`, `RTIME`,STRIKES,ATOTAL)
...
m <- left_join(m,select(data_b,-RTIME) ,by=c("AIRPORT ID" = "AIRPORT ID", "DATE"="DATE"))


### Step 9: Clean up column labels for clarity
m <- m %>%
  rename(`FLTS HIT` = STRIKES, `FLTS NOT HIT` = ATOTAL,
         BIRDCOUNT = COUNT)
